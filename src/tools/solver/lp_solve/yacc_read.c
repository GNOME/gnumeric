...
static void error(int verbose, char const *string)
{
  if(Verbose >= verbose)
 ...
 */
void read_error(char const *string)
{
...
static int readinput(lprec *lp)
{
  ...
  for(i = Rows;i >= 0;i--) {
    ...
    if((lp != NULL) && (rp != NULL)) {
      ...
      if(i) {
        lp_solve_set_constr_type(lp, i, rp->relat);
        relat[i] = rp->relat;
      }
      lp_solve_set_rh(lp, i, rp->value);
      if (rp->range_relat >= 0)
  ...
  if((stat != 0) || (CALLOC(relat, Rows + 1, short) != NULL)) {
	...
	if(Rows) {
	  ...
	  for(row = 1; row <= Rows; row++)
	    lp_solve_set_constr_type(lp, row, relat[row]);
