   ...
   row index base is 0 (which suits lp_solve fine). */
{
  ...
  for(j = 1; j <= ncols; j++) {
    ...
    /* Process user columns */
      ...
      k = kk - lp->rows;
      i = lp->matA->col_end[k-1];
      ii= lp->matA->col_end[k];
      Tnz += ii-i;
#ifdef Paranoia
      if(i >= ii)
      ...
      matelem = lp->matA->col_mat + i;
      rn = (*matelem).row_nr;
      hold = 0;
      if((rn > 0) && includeMDO(usedpos, 0) && lp->modifyOF1(lp, kk, &hold, 1.0)) {
        if(!dotally)
          data[Bnz] = offset;
        Bnz++;
      }
