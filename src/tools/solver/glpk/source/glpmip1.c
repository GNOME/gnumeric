...
{     MIPTREE *tree;
...
#if 1
      /* check whether the objective is integral or not */
      tree->int_obj = 1;
      for (j = 1; j <= n; j++)
      {  coef = lpx_get_col_coef(mip, j);
         if (coef != 0.0)
         {  if (lpx_get_col_kind(mip, j) != LPX_IV)
            {  /* continuous variable with non-zero coefficient */
               tree->int_obj = 0;
               break;
            }
      ...
      tree->heir = 0;
      tree->found = 0;
      tree->best = ucalloc(1+m+n, sizeof(gnm_float));
      tree->msg_lev = 3;
      tree->tol_int = 1e-6;
      tree->tol_obj = 1e-7;
      tree->it_lim = -1;
...
{     int better = 1;
      ...
      {  gnm_float eps = tree->tol_obj * (1.0 + gnumabs(tree->best[0]));
         ...
         {  case LPX_MIN:
#if 1
               /* if the objective is integral, round its value before
                  comparison */
               if (tree->int_obj)
...
{     MIPCOL *col;
      ...
      {  /* retrieve a pointer to the j-th variable (column) */
         ...
         /* if the variable is fixed, its value is integer feasible by
            definition (because the solution is primal feasible) */
         ...
         /* if the variable is non-basic, its value is integer feasible
            by definition (because its bounds are integral) */
...
{     LPX *lp;
      ...
      /* add new rows */
      if (nrs > 0)
      {  IESITEM **ref = ucalloc(1+nrs, sizeof(IESITEM *));
         for (i = 1; i <= nrs; i++)
         {  int i_new = m + i;
...
{     int m = tree->orig_m;
      ...
      {  ies_get_col_info(tree->tree, tree->col[j]->col, NULL, &temp,
         ...
         if (tree->col[j]->intvar) temp = gnm_floor(temp + 0.5);
         tree->best[m+j] = temp;
      }
      return;
}
...
{     LPX *lp = ies_get_lp_object(tree->tree);
      ...
      {  MIPNODE *node;
         ...
         {  case 1:
               dn_node = node;
               set_new_bound(tree, col, 'U', gnm_floor(vx));
               break;
            case 2:
               up_node = node;
               set_new_bound(tree, col, 'L', gnm_ceil(vx));
               break;
            default:
               insist(pass != pass);
         }
