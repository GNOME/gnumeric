...
static gnm_float get_next_elem(void *_info, int *i, int *j)
...
{     IESITEM *item;
      ...
      {  /* number of added rows and columns is not very big, so they
         ...
         {  /* fill the last nrs rows of the constraint matrix */
            ...
            val = ucalloc(1+tree->n-ncs, sizeof(gnm_float));
         ...
         {  /* fill the last ncs columns of the constraint matrix */
            ...
            val = ucalloc(1+tree->m, sizeof(gnm_float));
...
#     define reallocate(type, ptr, max_len, len) \
      ...
      reallocate(gnm_float, tree->lb, 1+m_max+n_max, 1+m+n);
      reallocate(gnm_float, tree->ub, 1+m_max+n_max, 1+m+n);
      reallocate(gnm_float, tree->coef, 1+m_max+n_max, 1+m+n);
...
{     IESNODE *node;
      ...
      {  /* there are not so many new rows, so they can be filled
         ...
         val = ucalloc(1+tree->n, sizeof(gnm_float));
         for (i = (tree->m - nrs) + 1; i <= tree->m; i++)
         {  item = tree->item[i];
            len = 0;
            for (e = item->ptr; e != NULL; e = e->r_next)
