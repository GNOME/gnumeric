...
gog_2d_plot_update (GogObject *obj)
{
	...
	for (ptr = model->base.series ; ptr != NULL ; ptr = ptr->next) {
		...
		if (series->base.values[0].data != NULL) {
			...
			if (!finitegnum (tmp_min) || !finitegnum (tmp_max) ||
			    tmp_min > tmp_max) {
				tmp_min = 0;
			     ...
			     GogPlotBoundInfo *bounds)
{
	...
	if (axis == GOG_AXIS_X) {
		...
		bounds->is_discrete = model->x.minima > model->x.maxima ||
			!finitegnum (model->x.minima) ||
			!finitegnum (model->x.maxima);
...
gog_xy_view_render (GogView *view, GogViewAllocation const *bbox)
{
	...
	for (ptr = model->base.series ; ptr != NULL ; ptr = ptr->next) {
		...
		if (GOG_IS_BUBBLE_PLOT (model)) {
			...
			if ((! finitegnum (zmax)) || (!show_negatives && (zmax <= 0))) continue;
		...
		for (i = 1 ; i <= n ; i++) {
			...
			if (valid) {
				/* We are checking with finitegnum here because isinf
