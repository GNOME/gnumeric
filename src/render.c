...
RenderInfo *
render_info_new (StyleElement *e, guint len)
{
	RenderInfo *ri = g_new (RenderInfo, 1);
	
	ri->halign      = HALIGN_GENERAL;
	ri->valign      = VALIGN_CENTER;
	ri->orientation = ORIENT_HORIZ;
	ri->fit_in_cell = 0;

	if (len > STYLE_ELEMENT_MAX_BLANK) {
		gchar *name;
		int    bold, italic;
		double scale;
		
		if (e[STYLE_FONT_NAME].type)
			name = e[STYLE_FONT_NAME].u.font.name;
		else
			name = DEFAULT_FONT;
		if (e[STYLE_FONT_BOLD].type)
			bold = e[STYLE_FONT_BOLD].u.font.bold;
		else
			bold = 0;
		if (e[STYLE_FONT_ITALIC].type)
			italic = e[STYLE_FONT_ITALIC].u.font.italic;
		else
			italic = 0;
		if (e[STYLE_FONT_SCALE].type)
			scale =  e[STYLE_FONT_SCALE].u.font.scale;
		else
			scale = 1.0;

		if (bold | italic | (name != DEFAULT_FONT) | (scale != 1.0))
			ri->font = style_font_new (name, DEFAULT_SIZE,
						   scale, bold, italic);
		else {
			ri->font = gnumeric_default_font;
			style_font_ref (ri->font);
		}
	}

	/* Styles that will show up on blank cells */
	if (e[STYLE_COLOR_FORE].type)
		ri->fore_color = e[STYLE_COLOR_FORE].u.color.fore;
	else
		ri->fore_color = style_color_new (0, 0, 0);

	if (e[STYLE_COLOR_BACK].type)
		ri->back_color = e[STYLE_COLOR_BACK].u.color.back;
	else
		ri->back_color = style_color_new (0xffff, 0xffff, 0xffff);
