...
static void
cb_line_update_bounds (SheetObject *so, FooCanvasItem *view)
{
...
static GObject *
gnm_so_line_new_view (SheetObject *so, SheetControl *sc, gpointer key)
{
	...
	g_signal_connect_object (G_OBJECT (sol),
		...
		item, 0);
	return gnm_pane_object_register (so, item, cb_line_update_bounds);
}
