...
typedef struct _ClauseData ClauseData;
typedef struct _SortData SortData;
typedef struct _OrderBox OrderBox;
typedef struct _SortFlow SortFlow;
...
static OrderBox *
order_box_new(GtkWidget * parent, const gchar *frame_text, GList *names, gboolean empty)
{
	OrderBox *orderbox;
	...
	gtk_box_pack_start(GTK_BOX(parent), GTK_WIDGET(orderbox->main_frame), FALSE, TRUE, 0);
...
{
	g_return_if_fail(orderbox);
	if (orderbox->main_frame)
		gtk_container_remove(GTK_CONTAINER(orderbox->parent), orderbox->main_frame);
			...
			for (lp2 = 0; lp2 < divisions; lp2++){
				...
				if (cell) {
					if (columns)
						...
						sheet_cell_add(sheet, cell, start_col + lp, start_row + lp2);
				}
			}
...
static GList *
cell_sort_col_name_list (Sheet *sheet, int start_col, int end_col, int row, gboolean header)
{
	gchar *str;
	GList *list;
	int i;
...
static GList *
cell_sort_row_name_list (Sheet *sheet, int start_row, int end_row, int col, gboolean header)
{
	gchar *str;
	GList *list;
	int i;
	...
	for (i=0; i<sf->num_clause; i++) {
		if (sf->header) {
			if (sf->columns)
				...
				gtk_combo_set_popdown_strings(GTK_COMBO(sf->clauses[i]->rangetext), sf->rownames_header);	
		} else {
			if (sf->columns)
				gtk_combo_set_popdown_strings(GTK_COMBO(sf->clauses[i]->rangetext), sf->colnames_plain);
			else
				gtk_combo_set_popdown_strings(GTK_COMBO(sf->clauses[i]->rangetext), sf->rownames_plain);
		}	
		if (i > 0)
