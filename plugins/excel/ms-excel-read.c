	...
	for (; ms_biff_query_next (q) ;
		...
		case BIFF_SCL:
			if (q->length == 4) {
				...
				double const zoom = (double)GSF_LE_GET_GUINT16 (q->data) /
...
{
	guint16	numTabs = GSF_LE_GET_GUINT16 (q->data);
	guint8 encodeType = GSF_LE_GET_GUINT8 (q->data + 2);
